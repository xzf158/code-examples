 ...
 * Active (after active state set)
 * Inactive (after inactive state set)
 * Unloaded
 *
 * In nearly all cases, the lifecycle progresses down the list. The one exception is that
 * an inactive component can be made active again. Otherwise, the order must be followed.
 *
 */
    ...
    extends MovieClip
{
    ...
    /**
     * Current component state. Can be "nostate", "unloaded", "loaded", "initialized", "active", "inactive"
     */
     ...
     *  state: the current state of the component. (loaded, initialized, active, or inactive)
     ...
     *   state: The components current state.
     *   child: If is is a child who is affected, this will contain the child affected and its state. (This is
    ...
    flite function stateChange(info:Object):void {
        ...
        //var affectedChildKey:String = child.key;
        //var affectedChildState:String = child.state;
    }
    ...
    private function render(rect:Rectangle):void {
        ...
        var rectangleAlpha:Number = _config.getNumber("alpha_param_name"); //Optionally, _config.map.alpha_param_name;
